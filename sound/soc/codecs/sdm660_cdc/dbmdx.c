...
extern struct snd_soc_codec *platform_codec;
extern int enable_mic_bias(bool enable);
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	printk("dbmdx_amodel_load_set value=%d\n",value);
				     ...
				     struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0]=4; /* Set analog mic only  tangshouxing 10/30 */
...
const struct snd_kcontrol_new dbmdx_va_snd_controls[] = {
	/*
	 * VA mixer controls
	 */
					...
					bool to_start_buffering)
{
	...
	if (p->pdata->send_uevent_on_detection) {
...
#ifdef NUANCE_FW_DETECTION_STATS
		if (p->va_backlog_length > (int)frames * 10 ) {
			dev_err(p->dev,
			...
			start_passphrase = p->va_backlog_length -
		...
		"VOICE_WAKEUP EVENT_ID=%d START=%d LENGTH=%d SCORE=%d",
			event_id,
			start_passphrase,
			length_in_sample*10,
			score);
...
static int dbmdx_platform_probe(struct platform_device *pdev)
{
	...
	gpio_direction_output(DBMDX_ENABLE_GPIO, 1);

	printk("%s,dbmdx_enable_gpio=%d\n",__func__,gpio_get_value(DBMDX_ENABLE_GPIO));
